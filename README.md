# Assessment: Real-World Application - Part B
...
| Library | Description |
| ----------- | ----------- |
| axios | Axios is a HTTP Client for the browser and Node JS.  It is being used in our backend to handle request from the frontend quering the database in the backend. |
| body-parser | "body-parser" is a Node JS middleware that enable data to be transmitted in the body of the HTTP request. It enables the backend part of the web application to receive data being passed in the body of the HTTP request packets. | 
| cors | CORS stands for Cross-Origin Resource Sharing.  It is a mechanism for managing request for resource outside of the origin.  It specifies not just who can access the resource and how the resource can be accessed.  The NPM library with the same name, "cors", is used in our backend to allow our frontend and backend to communicate to each other, especially the frontend accessing resources such as data, images, etc. |
| date-and-time | "date-and-time" package allows developers to easily manipulate the date and time.  It is used to subtract two dates, in order to determin if the password reset token has expired. |
| dotenv    | "dotenv" allows the environment file of the server to be accessed programmatically in the code.  Sensitive data such as username, password, API-Key, secret key to AWS or MongoDB, are stored in the environment file to avoid web crawlers sniffing them out and use them for malicious purpose.  "dotenv" makes it easy to retrieve such sensitive information. |
| express   | Express JS is a simplistic, unopinionated web framework for Node JS.  It enables a web server to be easily created and provide backend API services.  Express JS is being used to build the backend infrastructure. |
| jsonwebtoken  | JSON Web Token (JWT) is a compact, URL-safe means of representing claims or rights to be transmitted between two parties.  The claims in a JWT are encoded as a JSON object digitally encrypted with a Message Authentication Code (MAC).  The client upon receiving the token, can store it locally and use it to request access to restricted routes of the server. |
